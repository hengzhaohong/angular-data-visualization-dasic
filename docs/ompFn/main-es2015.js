...
const appRoutes = [
    {
        path: 'status',
        loadChildren: () => Promise.resolve(/*! import() */).then(__webpack_require__.bind(null, /*! ./status/status.module */ "./src/app/status/status.module.ts")).then(mod => mod.StatusModule)
    },
    {
        path: 'price',
        loadChildren: () => Promise.resolve(/*! import() */).then(__webpack_require__.bind(null, /*! ./price/price.module */ "./src/app/price/price.module.ts")).then(mod => mod.PriceModule)
    },
    {
        path: '',
        redirectTo: '/price',
...
/* harmony import */ var highcharts_angular__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! highcharts-angular */ "./node_modules/highcharts-angular/fesm2015/highcharts-angular.js");
 ...
 * Highchart 的通用初始配置
 */
const defaultGlobalChartOptions = {
    credits: {
        href: null
    },
    xAxis: {
 ...
 * @param count 序列长度
 */
const genTestData = (startWith = 520, count = 5000) => {
    const testData = [];
...
let PriceDashboardComponent = class PriceDashboardComponent {
...
let PriceRoutingModule = class PriceRoutingModule {
};
...
const genTestData = (startWith = 52, count = 5000) => {
    const testData = [];
    for (let i = 0; i < count; i++) {
        if (i === 0) {
...
let DataStatusDashboardComponent = class DataStatusDashboardComponent {
    // chartCallback = function (chart) { ... } // optional function, defaults to null
    // optional boolean, defaults to false
    constructor() {
        this.title = 'ompFn';
...
let StatusRoutingModule = class StatusRoutingModule {
};
